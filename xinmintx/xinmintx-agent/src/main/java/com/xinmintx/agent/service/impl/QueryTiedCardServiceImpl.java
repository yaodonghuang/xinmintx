...
/**
 * @ClassName:.QueryTiedCardServiceImpl
 * @author:chf
 * @Date:2019/12/24：10:44
 * @developerKits： win 10     jdk1.8
 */
@Service
public class QueryTiedCardServiceImpl implements QueryTiedCardService {
    @Override
    public ResultCode queryTiedCard(Map<String, String> paramMap) {
        // 初始化Map
        initMap(paramMap);
        // 生成post方法
        return pay(paramMap);
    ...
    private void initMap(Map<String, String> paramMap) {
        ...
        paramMap.put("postUrl", "https://www.56zhifu.com/user/BindCardQuery.do");
    }
    // 调用支付接口
    private ResultCode pay(Map<String, String> paramMap) {
        ...
        try {
            ...
            // 处理返回结果
            String[] bankNames = info.split("BankName");
            String[] split = bankNames[1].split("\"");
            rc.setMsg(split[1]);//截取开户行名称
