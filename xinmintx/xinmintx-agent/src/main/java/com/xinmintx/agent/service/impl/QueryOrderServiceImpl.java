...
import com.xinmintx.agent.service.QueryOrderService;
...
/**
 * @ClassName:.QueryOrderServiceImpl
 * @author:chf
...
public class QueryOrderServiceImpl implements QueryOrderService {
    ...
    @Override
    public ResultCode queryOrder(Map<String, String> paramMap) {
        // 初始化Map
        initMap(paramMap);
        // 生成post方法
        return pay(paramMap);
    ...
    private void initMap(Map<String, String> paramMap) {
        ...
        paramMap.put("postUrl", "https://www.56zhifu.com/user/warrantrTransQuery.do");
    }
    // 调用支付接口
    private ResultCode pay(Map<String, String> paramMap) {
        ...
        try {
            ...
            // 处理返回结果
            rc.setData(XMLConvertor.xml2Map(info));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return rc;
    ...
    private PostMethod initPostValue(Map<String, String> paramMap) {
        ...
        String orderType = paramMap.get("orderType");// 订单类型
        String orderId = paramMap.get("orderId");// 订单Id
        ...
        post.addParameter("orderType", orderType);
