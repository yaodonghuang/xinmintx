...
import com.xinmintx.factory.model.PayInfo;
            ...
            if (root.attributeValue("retCode").equals("0001")) {
                ...
                for (int i = 0; i < traInfoList.size(); i++) {
                    ...
                    if (temp.getNodeType() == 1) {
                        ...
                        if (tempElement.getName().equals("bankList")) {
                            List<Map<String, String>> bankList = new ArrayList<>();
                            List nodeList = tempElement.elements();
                            for (int j = 0; j < nodeList.size(); j++) {
                                Element nodeTemp = (Element) nodeList.get(j);
                                Iterator<Attribute> valueIt = nodeTemp.attributeIterator();
                                Map<String, String> valueMap = new HashMap<>();
                                while (valueIt.hasNext()) {
                                    Attribute at = valueIt.next();
                                    valueMap.put(at.getName(), at.getValue());
                                }
                                bankList.add(valueMap);
                            }
                            rtnMap.put(tempElement.getName(), bankList);
                        }
                    }
                }
