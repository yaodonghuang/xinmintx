...
import com.xinmintx.system.mapper.MemberLineMapper;
import com.xinmintx.system.mapper.MemberTreeMapper;
...
public class MemberTreeServiceImpl implements IMemberTreeService {
    ...
    public List<Integer> getMemberId(int id) {
        ...
        if (org.apache.commons.lang3.StringUtils.isNotEmpty(upperLine)) {
        ...
        if (m1 != null && m1.getMemberId() == null) {
            ...
            MemberTree m2 = memberTreeMapper.getUpperMemberId(m1.getMemberId().intValue());
            if (m2 == null) {
                // 删除树,从头开始计算
                memberLineMapper.deleteMemberLineById(getDeleteId);
                return getMemberId(getDeleteId);
            ...
            idList.add(m1.getMemberId().intValue());
            idList.add(m2.getMemberId().intValue());
            return idList;
        }
        return null;
    }
        ...
        if (org.apache.commons.lang3.StringUtils.isNotEmpty(treeCode)) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (m != null && m.getMemberId() != null) {
                    id = m.getMemberId().intValue();
                    idList.add(m.getMemberId().intValue());
                    if (m.getTreeId().intValue() == 1) {
                        ifTop = true;
                        topId = m.getMemberId().intValue();
                        break;
                    }
                } else {
                    break;
        ...
        if (mtList != null && mtList.size() > 0) {
            ...
            if (nextMtList != null && nextMtList.size() > 0) {
                for (MemberTree nextMt : nextMtList) {
                    // 下游递归查询
                    Integer nextCount = getNextMemberQty(nextMt.getMemberId().intValue());
    ...
    private static void dealData(Integer ifExistsMember, int id, String treeCode,
        ...
        if (ifExistsMember != null && ifExistsMember == 1 && org.apache.commons.lang3.StringUtils.isNotEmpty(treeCode)) {
        ...
        } else if ((ifExistsMember == null || ifExistsMember == 0) && org.apache.commons.lang3.StringUtils.isNotEmpty(treeCode)) {
            // 不存在且treeCode不为空,则更新
            Integer ifExistsTreeCode = memberTreeMapper.ifExistsTreeCode(treeCode);
            if (ifExistsTreeCode == null) {
                memberTreeMapper.setTreeNoForMember(id, treeCode);
        ...
        } else if ((ifExistsMember == null || ifExistsMember == 0) && org.apache.commons.lang3.StringUtils.isEmpty(treeCode)) {
    ...
    private static Member initMember(int i, List<MemberTree> mtList) {
        ...
        mb.setGender(1);
        mb.setIdcard("");
        mb.setMemberType(Long.valueOf(6));
        mb.setIsReview(1);
    ...
    {
        return memberTreeMapper.selectMemberTreeById(treeId);
    }
    ...
    {
        return memberTreeMapper.selectMemberTreeList(memberTree);
    }
    ...
    {
        memberTree.setCreateTime(DateUtils.getNowDate());
        return memberTreeMapper.insertMemberTree(memberTree);
    }
    ...
    {
        memberTree.setUpdateTime(DateUtils.getNowDate());
        return memberTreeMapper.updateMemberTree(memberTree);
    }
    ...
    {
        return memberTreeMapper.deleteMemberTreeByIds(Convert.toStrArray(ids));
    }
    ...
    {
        return memberTreeMapper.deleteMemberTreeById(treeId);
    }
