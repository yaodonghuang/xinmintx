...
(function ($) {
    $.extend({
        ...
        operate: {
			...
			endTask: function(id) {
				table.set();
				$.modal.confirm("确定关闭吗" + table.options.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? table.options.removeUrl : table.options.endTaskUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
			...
			startTask: function(id) {
				table.set();
				$.modal.confirm("确定开启" + table.options.modalName + "定时器？", function() {
					var url = $.common.isEmpty(id) ? table.options.startTaskUrl : table.options.startTaskUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
			...
			changeTask: function(id) {
				table.set();
				$.modal.confirm("确定变更" + table.options.modalName + "定时器？", function() {
					var url = $.common.isEmpty(id) ? table.options.changeTaskUrl : table.options.changeTaskUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
			...
			del: function(id) {
				table.set();
				$.modal.confirm("确定删除该条" + table.options.modalName + "信息吗？", function() {
					var url = $.common.isEmpty(id) ? table.options.delUrl : table.options.delUrl.replace("{id}", id);
					if(table.options.type == table_type.bootstrapTreeTable) {
						$.operate.get(url);
					} else {
						var data = { "ids": id };
