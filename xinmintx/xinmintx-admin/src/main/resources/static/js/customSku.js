...
// var valueNum="";
// for(var i=0;i<4;i++){
//     specNum+=Math.floor(Math.random()*10);
//     valueNum+=Math.floor(Math.random()*10);
// }
var customPropId = 1 ;//specNum.parseInt();//自定义属性类型ID
var customPropValId = 1;//valueNum.parseInt();//自定义属性值id
$(function(){
	var maxPropid = $("#maxPropid").val();
	var maxPropvalid = $("#maxPropvalid").val();
	if (maxPropid != null && maxPropid !== "" && maxPropid.length > 0){
		customPropId = maxPropid;
	}
	if (maxPropvalid != null && maxPropvalid !== "" && maxPropvalid.length > 0){
		customPropValId = maxPropvalid;
	}
	//克隆SKU模板生成自定义sku
